...
export const findBlockByTag = (tag: string, typeDoc: any): any => {
  ...
  return comment?.blockTags.find((child: any) => child.tag === tag)
}
  ...
  if (parameter.type.type === 'union') {
    ...
    description =
      parameter.comment?.summary
        ?.map((comment: { kind: string; text: string }) => comment.text)
        .join() ?? ''
  } else if (parameter.type.declaration) {
    type = parameter.type.declaration.signatures[0].type.name
    description =
      parameter.type.declaration.signatures[0].comment?.summary[0]?.text
  } else {
